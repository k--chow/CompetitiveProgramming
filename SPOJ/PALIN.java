...
get left half substring(0, length/2 + 1) odd, 0, length/2 even
reverse it
set it to big integer
see if larger than (length/2, length) 
see if longer length
if larger, simply replace second half (length/2  to length), break
if longer length replace second half (length/2 to length), break
else 
take half into big integer and increment by one, replace half
